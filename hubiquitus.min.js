...
}for(i=0;i<len;i++){list[i].apply(this,args)}return this};return EventEmitter}();if(!window.hubiquitus)window.hubiquitus={};window.hubiquitus._Transport=function(){"use strict";var util=window.hubiquitus._util;var EventEmitter=window.hubiquitus._EventEmitter;var loggerManager=window.hubiquitus._loggerManager;var logger=loggerManager.getLogger("hubiquitus:transport");var connTimeout=1e4;var reconnDelay=3e3;var negoationTimeout=2e3;var heartbeatFreq=15e3;var Transport=function(){function Transport(options){EventEmitter.call(this);this.endpoint=null;options=options||{};this._sock=null;this._started=false;this._locked=false;this._events=new EventEmitter;this._autoReconnect=options.autoReconnect||false;this._shouldReconnect=false;this._reconnecting=false;this._negotiating=false;this._whitelist=["websocket","xdr-streaming","xhr-streaming","iframe-eventsource","iframe-htmlfile","xdr-polling","xhr-polling","iframe-xhr-polling","jsonp-polling"];this._heartbeatFreq=15e3;this._lastHeartbeat=-1}util.inherits(Transport,EventEmitter);Transport.prototype.connect=function(endpoint,cb){if(endpoint)this.endpoint=endpoint;logger.trace("connecting "+this.endpoint+"...");if(this._locked&&!this._reconnecting&&!this._negotiating||this._started){logger.warn(this._locked?"busy":"already started","; cant connect "+endpoint);this.emit("error",{code:this._locked?"BUSY":"ALREADYCONN"});return}cb&&this.once("connect",cb);if(!this._reconnecting){this._locked=true;if(this._autoReconnect)this._shouldReconnect=true}this._sock=new SockJS(this.endpoint,null,{protocols_whitelist:this._whitelist});this._sock.onopen=this._onopen.bind(this);this._sock.onclose=this._onclose.bind(this);this._sock.onmessage=this._onmessage.bind(this);var _this=this;setTimeout(function(){if(!_this._started)_this.emit("error",{code:"CONNTIMEOUT"})},connTimeout)};Transport.prototype.disconnect=function(cb){logger.trace("disconnecting...");if(this._locked&&!this._reconnecting&&!this._negotiating||!this._started){logger.warn(this._locked?"busy":"already stopped","; cant disconnect");this.emit("error",{code:this._locked?"BUSY":"ALREADYDISCONN"});return}cb&&this.once("disconnect",cb);this._locked=true;this._shouldReconnect=false;this._sock&&this._sock.close()};Transport.prototype.send=function(msg,cb){if(this._locked||!this._started){logger.warn(this._locked?"busy":"not started","; cant send message",msg);cb&&cb({code:this._locked?"BUSY":"NOTCONN"});return}var encodedMsg=encode(msg);encodedMsg&&this._sock.send(encodedMsg);cb&&cb()};Transport.prototype._onopen=function(){var _this=this;_this._started=true;_this._locked=false;this._negotiate(function(err){if(err){logger.warn("Protocol negotiation error");_this._sock&&_this._sock.close();_this.emit("error",err);return}_this._lastHeartbeat=Date.now();_this._checkConn();logger.info((_this._reconnecting?"reconnected":"connected")+" using "+_this._sock.protocol);_this.emit(_this._reconnecting?"reconnect":"connect");_this._reconnecting=false})};Transport.prototype._checkConn=function(){var _this=this;if(_this._started){if(Date.now()-(_this._lastHeartbeat+_this._heartbeatFreq+.5*_this._heartbeatFreq)>0){logger.debug("Last heartbeat too old : ",_this._lastHeartbeat," freq : ",_this._heartbeatFreq);_this._sock&&_this._sock.close();_this.emit("error",{code:"HBTIMEOUT"})}setTimeout(function(){_this._checkConn()},_this._heartbeatFreq)}};Transport.prototype._onclose=function(){this._sock=null;this._started=false;this._lastHeartbeat=-1;if(!this._shouldReconnect){this._locked=false;logger.info("disconnected");this.emit("disconnect")}else if(!this._reconnecting){this.emit("disconnect");this._reconnecting=true;var _this=this;(function reconnect(){if(_this._started||!_this._shouldReconnect)return;logger.info("reconnecting...");_this.connect();setTimeout(reconnect,reconnDelay)})()}};Transport.prototype._onmessage=function(e){logger.trace("received message",e.data);if(e.data==="hb"){this._lastHeartbeat=Date.now();this._sock.send("hb")}else{var msg=decode(e.data);if(msg.type==="negotiate"){return this._events.emit("negotiate",null,msg)}msg&&this.emit("message",msg)}};Transport.prototype._negotiate=function(cb){var _this=this;this._events.once("negotiate",function(err,msg){if(err){logger.trace("negotiation timeout");_.remove(_this._whitelist,function(item){return item==="websocket"});if(_this._sock.protocol==="websocket"){_this.disconnect(function(){_this._negotiating=false;_this.connect()})}else{cb&&cb({code:"NEGOTIATIONERR"})}this.emit("error",{code:_this._locked?"BUSY":"ALREADYDISCONN"});return}if(msg&&typeof msg.heartbeatFreq==="number"&&msg.heartbeatFreq>0)_this._heartbeatFreq=msg.heartbeatFreq;_this._negotiating=false;logger.trace("negotiation sucessful");cb&&cb()});setTimeout(function(){_this._events.emit("negotiate",{code:"TIMEOUT"})},negoationTimeout);_this._negotiating=true;logger.trace(this._sock.protocol+" protocol; Protocol negotiation");var negotiate=encode({type:"negotiate"});this._sock.send(negotiate)};return Transport}();function encode(data){var encodedData=null;try{encodedData=JSON.stringify(data)}catch(err){logger.warn("failed encoding data",data,err)}return encodedData}function decode(data){var decodedData=null;try{decodedData=JSON.parse(data)}catch(err){logger.warn("failed decoding data",data,err)}return decodedData}return Transport}();if(!window.hubiquitus)window.hubiquitus={};window.hubiquitus.Hubiquitus=function(){"use strict";var util=window.hubiquitus._util;var EventEmitter=window.hubiquitus._EventEmitter;var loggerManager=window.hubiquitus._loggerManager;var Transport=window.hubiquitus._Transport;var logger=loggerManager.getLogger("hubiquitus");var defaultSendTimeout=3e4;var maxSendTimeout=5*36e5;var authTimeout=3e3;function Hubiquitus(options){EventEmitter.call(this);options=options||{};this._transport=new Transport({autoReconnect:options.autoReconnect||false});this._events=new EventEmitter;this._authentified=false;this._authData=null;this.id=null;this._connected=false;var _this=this;var reconnecting=false;this._transport.on("error",function(err){_this.emit("error",err)});this._transport.on("connect",function(){reconnecting=false;_this._connected=true;_this._login(_this._authData)});this._transport.on("reconnect",function(){reconnecting=true;_this._connected=true;_this._login(_this._authData)});this._transport.on("disconnect",function(){_this._authentified=false;if(_this._connected)_this.emit("disconnect");_this._connected=false});this._transport.on("message",function(msg){switch(msg.type){case"req":_this._onReq(msg);break;case"res":_this._events.emit("res|"+msg.id,msg);break;case"login":_this._authentified=true;_this.id=msg.content.id;logger.info("logged in; identifier is",_this.id);_this.emit(reconnecting?"reconnect":"connect");break;default:logger.warn("received unknown message type",msg)}})}util.inherits(Hubiquitus,EventEmitter);Hubiquitus.logger=loggerManager;Hubiquitus.prototype.util=util;Hubiquitus.prototype.connect=function(endpoint,authData){this._authData=authData;this._transport.connect(endpoint);return this};Hubiquitus.prototype.disconnect=function(){this._transport.disconnect();return this};Hubiquitus.prototype.send=function(to,content,timeout,cb){if(_.isFunction(timeout)){cb=timeout;timeout=defaultSendTimeout}var _this=this;timeout=_.isNumber(timeout)?timeout:maxSendTimeout;var req={to:to,content:content,id:util.uuid(),date:(new Date).getTime(),type:"req"};if(cb)req.cb=true;_this._events.once("res|"+req.id,function(res){_this._onRes(res,cb)});setTimeout(function(){_this._events.emit("res|"+req.id,{err:{code:"TIMEOUT"},id:req.id})},timeout);logger.trace("sending request",req);this._transport.send(req,function(err){if(err)_this._events.emit("res|"+req.id,{err:{code:"TECHERR"},id:req.id})});return this};Hubiquitus.prototype._onReq=function(req){logger.trace("processing message",req);var _this=this;req.reply=function(err,content){var res={to:req.from,id:req.id,err:err,content:content,type:"res"};_this._transport.send(res)};try{this.emit("message",req)}catch(err){logger.warn("processing request error",{req:req,err:err})}};Hubiquitus.prototype._onRes=function(res,cb){logger.trace("processing response",res);try{cb&&cb(res.err,res)}catch(err){logger.warn("processing response error",{res:res,err:err})}};Hubiquitus.prototype._login=function(authData){var _this=this;setTimeout(function(){if(!_this._authentified){_this.emit("error",{code:"AUTHTIMEOUT"});_this.disconnect()}},authTimeout);var msg={type:"login",authData:authData};logger.trace("try to login",msg);this._transport.send(msg)};return Hubiquitus}()})();