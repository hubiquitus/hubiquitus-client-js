...
define(
    ...
    function(Strophe, codes, sessionStorageAux){
        ...
        var hSessionBosh = function(publisher, password, cb, opts) {
            ...
            this.domain = publisher.split('@')[1];

            this.pubsub =  "pubsub." + this.domain;
            this.hnode = 'hnode@' + this.domain;
        ...
        hSessionBosh.prototype = {
            ...
            onStatus: function(isHStatus, value, msg)
            {
                ...
                function _onConnection(){
                    ...
                    self.conn.addHandler(self.handleHStructure.bind(self), null, 'message',
                        null, null, null, {matchBare: true});
                    ...
                    self.retryIntervals = self.opts.retryIntervals; //Resets the reconnection counter
                }
                function _onError(){
            ...
            //Handles a hubiquitus XMPP message
            handleHStructure: function(stanza){
                ...
                if(body){
                    ...
                    try{
                        var content = JSON.parse(body.textContent.replace(/&quot;/g,'"'));
                    }catch(err){}

                    switch(body.getAttribute('type')){
                        case 'hmessage':
                            if(content)
                                this.callback('hMessage', content);
                            break;
                    }
                }
            ...
            sendhMessage : function(hMessage){
                var msg = $msg({to: hMessage.actor})
                    .c('hbody', {type: 'hmessage'}, JSON.stringify(hMessage));
                this.conn.send(msg);
            }
