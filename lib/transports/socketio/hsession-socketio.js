...
define(
    ...
    function(codes, sessionStorageAux){
         ...
         */
        var hSessionSocketIO = function(opts, callback){
            this.params = opts;
            ...
            if(opts.route.length > 0){
                ...
                this.params.host = splitted[0];
                this.params.port = splitted[1];
            }
        };
        ...
        hSessionSocketIO.prototype.connect = function(){
            ...
            var on_socket = function(){
                ...
                var re = new RegExp(self.params.publisher,"i");
                if( typeof sessionStorage.getItem('userid') === 'string' &&
                    sessionStorage.getItem('userid').search(re) != -1){
            ...
            this.socket.on('error', function(){
                self.on_status({status: statuses.Error, errorCode: errors.CONN_TIMEOUT});
            });
        };
        ...
        hSessionSocketIO.prototype.on_status = function(msg){
            if(msg){
                this.callback({context: contexts.hStatus, data: msg});
                ...
                if(msg.status == statuses.Connected){
                    ...
                    if(msg.errorCode == errors.AUTH_FAILED) return;
